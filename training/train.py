...
def loaders(data_root, batch_size=16):
    ...
    return (DataLoader(train, batch_size=batch_size, shuffle=True),
            DataLoader(val, batch_size=batch_size), train.classes)
...
def main():
    parser = argparse.ArgumentParser(description="Train an image classifier on a dataset organized as dataset/train and dataset/val.")
    parser.add_argument("--data-root", default=os.path.join(ROOT, "dataset"),
                        help="Path to the dataset root containing train/ and val/ subdirectories.")
    parser.add_argument("--epochs", type=int, default=12, help="Number of training epochs.")
    parser.add_argument("--batch-size", type=int, default=16, help="Batch size for training and validation.")
    args = parser.parse_args()

    if not os.path.isdir(args.data_root):
        raise FileNotFoundError(f"Dataset root not found: {args.data_root}\nCreate a dataset folder with train/ and val/ subdirectories, or pass --data-root.")
    if not os.path.isdir(os.path.join(args.data_root, "train")):
        raise FileNotFoundError(f"Train folder not found: {os.path.join(args.data_root, 'train')}")
    if not os.path.isdir(os.path.join(args.data_root, "val")):
        raise FileNotFoundError(f"Validation folder not found: {os.path.join(args.data_root, 'val')}")

    epochs = args.epochs
    tl, vl, classes = loaders(args.data_root, batch_size=args.batch_size)
